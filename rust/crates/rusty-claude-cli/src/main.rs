...
const INITIAL_STREAM_STALL_TIMEOUT: Duration = Duration::from_secs(20);
...
██║  ██║██╔═██╗ ██║   ██║\n\
██████╔╝██║  ██╗╚██████╔╝\n\
╚═════╝ ╚═╝  ╚═╝ ╚═════╝ \x1b[0m \x1b[38;5;34mCode\x1b[0m 🦞\n\n\
  \x1b[2mModel\x1b[0m            {}\n\
        ...
        self.runtime.block_on(async {
            ...
            for attempt in 1..=max_attempts {
                ...
                match result {
                    ...
                    Err(error) if error.to_string().contains("initial stream stall") => {
                        // Some OpenAI-compatible backends can keep a streaming
                        // socket open without delivering the first event.
                        // Fall back to non-streaming so the turn can finish.
                        return self.consume_non_stream(&message_request).await;
                    }
                    Err(error) if error.to_string().contains("stream idle stall") => {
                        // Some providers emit one stream event and then stall.
                        // Fall back to non-streaming instead of waiting forever.
                        return self.consume_non_stream(&message_request).await;
                    }
                    Err(error)
...
impl AnthropicRuntimeClient {
    ...
    async fn consume_stream(
        ...
        loop {
            let timeout = if !received_any_event {
                if apply_stall_timeout {
                    POST_TOOL_STALL_TIMEOUT
                } else {
                    INITIAL_STREAM_STALL_TIMEOUT
                }
            } else {
                STREAM_IDLE_TIMEOUT
            };
            let next = match tokio::time::timeout(timeout, stream.next_event()).await {
                Ok(inner) => inner.map_err(|error| {
                    RuntimeError::new(format_user_visible_api_error(&self.session_id, &error))
                })?,
                Err(_elapsed) => {
                    let label = if !received_any_event {
                        if apply_stall_timeout {
                            "post-tool stall"
                        } else {
                            "initial stream stall"
                        }
                    } else {
                        "stream idle stall"
                    };
                    return Err(RuntimeError::new(format!(
                        "{label}: model did not respond within timeout"
                    )));
                }
            };
        ...
        self.consume_non_stream(message_request).await
    }

    async fn consume_non_stream(
        &self,
        message_request: &MessageRequest,
    ) -> Result<Vec<AssistantEvent>, RuntimeError> {
        let mut stdout = io::stdout();
            ...
            let content = message
                ...
                .filter_map(|block| match block {
                    ...
                    ContentBlock::Thinking { thinking, signature } => {
                        (role == "assistant").then(|| InputContentBlock::Thinking {
...
UU conflicted.rs",
    ...
    #[test]
    fn convert_messages_preserves_assistant_thinking_blocks() {
        let messages = vec![
