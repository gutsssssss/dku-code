...
const DEEPSEEK_V4_REASONING_ENV: &str = "CLAW_DEEPSEEK_V4_REASONING";
...
fn deepseek_v4_reasoning_enabled() -> bool {
    parse_bool_env(std::env::var(DEEPSEEK_V4_REASONING_ENV).ok().as_deref())
}
...
/// Whether assistant history should echo hidden reasoning as `reasoning_content`.
///
/// DeepSeek currently rejects requests that include `reasoning_content` in
/// subsequent input messages. Keep this disabled until provider behavior is
/// confirmed to require it again.
#[must_use]
pub fn model_requires_reasoning_content_in_history(model: &str) -> bool {
    is_deepseek_v4_model(model) && deepseek_v4_reasoning_enabled()
}
...
fn build_chat_completion_request_for_base_url(
    ...
    let include_reasoning_history = model_requires_reasoning_content_in_history(wire_model);
    for message in &request.messages {
        messages.extend(translate_message_with_reasoning_history(
            message,
            wire_model,
    ...
    // Keep DeepSeek v4 in stable mode by default (thinking disabled). Users can
    // opt in to thinking + reasoning history by exporting
    // `CLAW_DEEPSEEK_V4_REASONING=1`.
    if is_deepseek_v4_model(wire_model) {
        payload["thinking"] = if deepseek_v4_reasoning_enabled() {
            json!({ "type": "enabled" })
        } else {
            json!({ "type": "disabled" })
        };
...
pub fn translate_message(message: &InputMessage, model: &str) -> Vec<Value> {
    translate_message_with_reasoning_history(
        message,
        model,
...
fn translate_message_with_reasoning_history(
    ...
    match message.role.as_str() {
        "assistant" => {
            ...
            let include_reasoning = include_reasoning_history && !reasoning.is_empty();
...
mod tests {
    use super::{
        ...
        openai_tool_choice, parse_bool_env, parse_tool_arguments, OpenAiCompatClient,
        OpenAiCompatConfig, StreamState, DEEPSEEK_V4_REASONING_ENV,
    };
    use crate::error::ApiError;
    use crate::types::{
        ContentBlockDelta, ContentBlockDeltaEvent, ContentBlockStartEvent, ContentBlockStopEvent,
    ...
    #[test]
    fn model_requires_reasoning_content_in_history_is_disabled_by_default() {
        let _lock = env_lock();
        let previous = std::env::var(DEEPSEEK_V4_REASONING_ENV).ok();
        std::env::remove_var(DEEPSEEK_V4_REASONING_ENV);
        ...
        if let Some(value) = previous {
            std::env::set_var(DEEPSEEK_V4_REASONING_ENV, value);
        }
    }
    ...
    fn deepseek_v4_pro_request_omits_reasoning_content_for_assistant_history() {
        ...
        // Then reasoning_content is omitted on the assistant message.
        let assistant = &payload["messages"][0];
        assert!(assistant.get("reasoning_content").is_none());
    ...
    #[test]
    fn deepseek_v4_request_disables_thinking_by_default() {
        let _lock = env_lock();
        let previous = std::env::var(DEEPSEEK_V4_REASONING_ENV).ok();
        std::env::remove_var(DEEPSEEK_V4_REASONING_ENV);

        let request = MessageRequest {
            model: "xai/deepseek-v4-flash".to_string(),
            max_tokens: 128,
    ...
    #[test]
    fn deepseek_v4_request_enables_thinking_when_env_toggle_is_on() {
        let _lock = env_lock();
        let previous = std::env::var(DEEPSEEK_V4_REASONING_ENV).ok();
        std::env::set_var(DEEPSEEK_V4_REASONING_ENV, "1");

        let request = MessageRequest {
            model: "xai/deepseek-v4-pro".to_string(),
            max_tokens: 128,
            messages: vec![InputMessage::user_text("hello")],
            stream: true,
            ..Default::default()
        ...
        assert!(model_requires_reasoning_content_in_history("xai/deepseek-v4-pro"));
    ...
    #[test]
    fn deepseek_v4_tool_continuation_preserves_thinking_and_reasoning_content() {
        let _lock = env_lock();
        let previous = std::env::var(DEEPSEEK_V4_REASONING_ENV).ok();
        std::env::set_var(DEEPSEEK_V4_REASONING_ENV, "1");

        let request = deepseek_tool_continuation_request("xai/deepseek-v4-pro");
        let payload = build_chat_completion_request(&request, OpenAiCompatConfig::xai());
    ...
    #[test]
    fn non_deepseek_request_does_not_force_thinking_flag() {
        let request = MessageRequest {
    ...
    fn deepseek_tool_continuation_request(model: &str) -> MessageRequest {
        MessageRequest {
            model: model.to_string(),
            max_tokens: 100,
            messages: vec![
