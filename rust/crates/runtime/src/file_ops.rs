...
fn validate_workspace_boundary(resolved: &Path, workspace_root: &Path) -> io::Result<()> {
    if !path_is_within_workspace(resolved, workspace_root) {
...
fn path_is_within_workspace(resolved: &Path, workspace_root: &Path) -> bool {
    #[cfg(windows)]
    {
        let resolved_text = normalize_windows_boundary_path(resolved);
        let root_text = normalize_windows_boundary_path(workspace_root);
        return windows_path_is_same_or_descendant(&resolved_text, &root_text);
    }
...
#[cfg(windows)]
fn normalize_windows_boundary_path(path: &Path) -> String {
...
#[cfg(windows)]
fn windows_path_is_same_or_descendant(resolved: &str, workspace_root: &str) -> bool {
    if resolved == workspace_root {
        return true;
    }

    let mut prefix = workspace_root.to_owned();
    if !prefix.ends_with('\\') {
        prefix.push('\\');
    }
...
fn normalize_path(path: &str) -> io::Result<PathBuf> {
    ...
    candidate.canonicalize().map(normalize_windows_verbatim_path)
}
...
fn normalize_path_allow_missing(path: &str) -> io::Result<PathBuf> {
    ...
    if let Ok(canonical) = candidate.canonicalize() {
        return Ok(normalize_windows_verbatim_path(canonical));
    }
    ...
    if let Some(parent) = candidate.parent() {
        ...
        if let Some(name) = candidate.file_name() {
            return Ok(normalize_windows_verbatim_path(canonical_parent).join(name));
        }
    }

    Ok(normalize_windows_verbatim_path(candidate))
}
...
pub fn is_symlink_escape(path: &Path, workspace_root: &Path) -> io::Result<bool> {
    ...
    let canonical_root = workspace_root
        .canonicalize()
        .unwrap_or_else(|_| workspace_root.to_path_buf());
    Ok(!path_is_within_workspace(&resolved, &canonical_root))
}
...
mod tests {
    ...
    use super::{
        ...
        glob_search_in_workspace, grep_search, is_symlink_escape, read_file,
    ...
    #[test]
    #[cfg(windows)]
    fn windows_workspace_boundary_accepts_verbatim_and_nonverbatim_forms() {
        let resolved = PathBuf::from(r"C:\Users\Administrator\Desktop\work\project");
        let workspace = PathBuf::from(r"\\?\C:\Users\Administrator\Desktop\work");
        super::validate_workspace_boundary(&resolved, &workspace)
            .expect("mixed Windows path forms should be treated as same root");
    }
    ...
    #[test]
    #[cfg(windows)]
    fn glob_search_handles_verbatim_workspace_roots() {
        let dir = temp_path("glob-verbatim-root");
        std::fs::create_dir_all(&dir).expect("directory should be created");
        let file = dir.join("demo.txt");
        write_file(file.to_string_lossy().as_ref(), "ok").expect("file write should succeed");

        let verbatim_root = PathBuf::from(format!(r"\\?\{}", dir.to_string_lossy()));
        let output =
            glob_search_in_workspace("**/*", Some(dir.to_string_lossy().as_ref()), &verbatim_root)
                .expect("glob should succeed with verbatim workspace root");

        assert_eq!(output.num_files, 1);
        assert!(output.filenames.iter().any(|entry| entry.ends_with("demo.txt")));
    }
